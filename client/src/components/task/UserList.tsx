...
// Define the User type based on the data structure used in the user object
interface User {
  _id: string;
  name: string;
        ...
        <div className='relative mt-1'>
          ...
          >
            <Listbox.Options className='z-50 absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm'>
              {data.map((user, index) => (
                <Listbox.Option
                  key={user._id} // Using user._id as the key for uniqueness
                  className={({ active }) =>
